            ...
            if (xhr.status >= 200 && xhr.status < 300) {
                ...
                if (User.code == 200) {
                    ...
                    xhr1.onreadystatechange = function () {
                        if (xhr1.readyState == 4) {
                            if (xhr1.status >= 200 && xhr1.status < 300) {
                                var xhr1Response = xhr1.responseText;
                                console.log(xhr1.xhr1Response);
                                console.log(xhr1.xhr1Response.account);
                                console.log(xhr1.xhr1Response.profile);
                                console.log(xhr1.xhr1Response.profile.avatarUrl);
                                console.log(xhr1.xhr1Response.profile.nickname);
                                console.log(xhr1.xhr1Response.profile.level);
                            }
                        }
                    }
                    //获取用户详情后同步到页面中
                    ...
                    HeadPic.src = xhr1.response.profile.avatarUrl;
                    name.innerHTML = xhr1.response.profile.nickname;
                    level.innerHTML = xhr1.response.profile.level;
                    //检测登陆状态
                    let xhr = new XMLHttpRequest;
                    xhr.open('GET', 'http://localhost:3000/login/status');
                    xhr.send();
