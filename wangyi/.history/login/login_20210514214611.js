            ...
            if (xhr.status >= 200 && xhr.status < 300) {
                ...
                if (User.code == 200) {
                    ...
                    xhr1.onreadystatechange = function () {
                        if (xhr1.readyState == 4) {
                            if (xhr1.status >= 200 && xhr1.status < 300) {
                                var xhr1Response = JSON.parse(xhr1.responseText);
                                console.log(xhr1Response);
                                console.log(xhr1Response.account);
                                console.log(xhr1Response.account.id);
                                console.log(xhr1Response.profile);
                                console.log(xhr1Response.profile.userId);
                                console.log(xhr1Response.profile.avatarUrl);
                                console.log(xhr1Response.profile.nickname);
                                //获取用户详情后同步到页面中
                                //同步头像
                                let HeadPic = document.getElementsByClassName('HeadPic')[0];
                    ...
                    let xhr2 = new XMLHttpRequest;
                    xhr2.open('GET', 'http://localhost:3000/user/detail?uid=' + JSON.parse(xhr1.responseText).account.id);
                    xhr2.send();
