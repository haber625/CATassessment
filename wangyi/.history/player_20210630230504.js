...
let playingLine = function () {
    ...
    line_cir.addEventListener('mousedown', () => {
        //先停止播放音乐
        // audioPlayer.pause();
        ...
        let aaa = function (event) {
            ...
            var distance = xLeft - left;//鼠标点击时到进度条左侧的距离
            play_lineCur.style.width = distance + "px";
            // audioPlayer.currentTime = distance / 493 * audioPlayer.duration;//改变播放时间
        }

        line_cir.addEventListener('mousemove', aaa);



    })
    line_cir.addEventListener('mouseup', () => {
        clearInterval(timer1);
        // audioPlayer.currentTime = play_lineCur.style.width / 493 * audioPlayer.duration;
        //继续播放音乐
        audioPlayer.play();
        btns_play.style.backgroundPositionY = "-165px";
        // line_cir.onmousemove = null;
        line_cir.removeEventListener('mousemove', aaa)
