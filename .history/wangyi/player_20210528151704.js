...
console.log(song_rise);
//获取榜单
get('http://localhost:3000/toplist', '', function (data) {
    console.log(data);
})
//获取飙升榜单
let hotList_xhr = get('http://localhost:3000/playlist/detail?id=19723756', '', function (data) {
    console.log(data.playlist.tracks);
    for (var i = 0; i < 10; i++) {
        //更新歌名
        song_rise[i].innerHTML = data.playlist.tracks[i].name;
        //根据id获取url
        get('http://localhost:3000/song/url', 'id=' + data.playlist.tracks[i].id, function (data) {
            for (var i = 0; i < 10; i++) {
                song_rise[1].addEventListener('click', function () {
        ...
        // song_rise[i].addEventListener('click', function () {
        //     get('http://localhost:3000/song/url', 'id=' + song_id, function (data) {
        //         audioPlayer.src = "" + data.data[0].url;
