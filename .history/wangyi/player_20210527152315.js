...
let playingLine = function () {
    ...
    //点击进度条改变歌曲时间
    let play_lineCur_content = document.getElementsByClassName('play-lineCur-content')[0];

    play_lineCur_content.addEventListener('click', function (event) {
        var left = getAbsLeft(play_lineCur_content);//进度条到页面左边的距离
        var xLeft = event.clientX;//鼠标点击时到页面右侧的距离
        var distance = xLeft - left;//鼠标点击时到进度条左侧的距离
        audioPlayer.currentTime = distance / lenth * audioPlayer.duration;
    })
    ...
    // play_lineCur.addEventListener('mousedown', function (event) {
    //     var x = event.clientX;
    //     console.log(line_cir.offsetLeft + 9);//小圆圈到进度条的距离
    //     console.log(x);//鼠标点击时的坐标，距离页面的距离

    //     line_cir.addEventListener('mousemove', function (event) {
    //         console.log(event.clientX);
    //     })
    // })
    // play_lineCur.addEventListener('mouseup', function () {
    //     console.log(1);
    //     line_cir.removeEventListener('mousemove', function () { })
    // })
    //移动小圆圈时的事件
