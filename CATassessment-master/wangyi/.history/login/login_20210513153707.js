...
loginClose.onclick = function () {
    ...
    document.body.style.overflow = "roll";
...
login.onclick = function () {
    ...
    xhr1.open('GET', 'https://netease-cloud-music-api-murex-chi.vercel.app/login/qr/key');
    xhr1.send();

    xhr1.onreadystatechange = function () {
        if (xhr1.readyState === 4) {
            if (xhr1.status >= 200 && xhr1.status < 300) {
                // key = xhr1.responseText;
            }
        }
    }
    ...
    xhr2.open('GET', 'https://netease-cloud-music-api-murex-chi.vercel.app/login/qr/create?key=' + xhr1.response.key + '&&' + 'qrimg=1');
    xhr2.send();
    ...
    xhr2.onreadystatechange = function () {
        if (xhr2.readyState === 4) {
            if (xhr2.status >= 200 && xhr2.status < 300) {
                ...
                console.log(xhr2Response.data);
                console.log(xhr2Response.data.qrimg);
                console.log(qr);
    ...
    xhr3.open('GET', 'https://netease-cloud-music-api-murex-chi.vercel.app/login/qr/check?key=' + xhr1.response.key);
    xhr3.send();
        ...
        if (xhr.readyState === 4) {
            if (xhr.status >= 200 && xhr.status < 300) {
                ...
                if (User.code == 200) {
                    ...
                    fetch('https://netease-cloud-music-api-murex-chi.vercel.app/user/account', { credentials: 'include' })
                    xhr1.open('GET', 'https://netease-cloud-music-api-murex-chi.vercel.app/user/account');
                    // xhr1.setRequestHeader("Authorization", document.cookie);
                    xhr1.send();
                    xhr1.onreadystatechange = function () {
                        if (xhr1.readyState == 4) {
                            if (xhr1.status >= 200 && xhr1.status < 300) {
                    ...
                    //刷新登陆状态
                    // let xhr1 = new XMLHttpRequest;
                    // xhr1.open('GET', 'https://netease-cloud-music-api-murex-chi.vercel.app/login/refresh');
                    // xhr1.setRequestHeader("Content-type", "application/json; charset=utf-8" + document.cookie);
                    ...
                    let xhr4 = new XMLHttpRequest;
                    xhr4.open('GET', 'https://netease-cloud-music-api-murex-chi.vercel.app/login/status');
                    xhr4.setRequestHeader("Content-type", "application/json; charset=utf-8" + document.cookie);
                    xhr4.send();
...
//微信，qq，微博登录的提示窗口
let u_mlg2_wx = document.getElementsByClassName('u-mlg2-wx')[0];
let u_mlg2_qq = document.getElementsByClassName('u-mlg2-qq')[0];
let u_mlg2_wb = document.getElementsByClassName('u-mlg2-wb')[0];
...
u_mlg2_wx.href = "javascript:;"
u_mlg2_wx.addEventListener('click', remindbox, false);
u_mlg2_wx.addEventListener('click', function () {
    if (u_official_terms.checked == false) {
        this.style.href = "javascript:;";
    }
}, false);
...
//     Logins.style.height = "310px";
// }
// while (email_login.style.display == "block") {
//     console.log("1");
//     Logins.style.height = "310px";
// }
