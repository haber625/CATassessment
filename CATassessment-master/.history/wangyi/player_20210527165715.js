...
let playingLine = function () {
    ...
    line_cir.addEventListener('mousedown', function () {
        var cirLeft = line_cir.offsetLeft;//小圆圈到进度条左侧的距离，从-9开始
        console.log(cirLeft);
        play_lineCur.addEventListener('mousemove', function Move() {
            console.log('move');
            var left = getAbsLeft(play_lineCur);//进度条到页面左边的距离
            var xLeft = event.pageX;//鼠标点击时到页面左侧的距离
            var distance = xLeft - left;//鼠标点击时到进度条左侧的距离
        ...
        line_cir.addEventListener('mouseup', function () {
            console.log('mouseup');
            play_lineCur.removeEventListener('mousemove', function Move() {
                console.log('move');
                var left = getAbsLeft(play_lineCur);//进度条到页面左边的距离
                var xLeft = event.pageX;//鼠标点击时到页面左侧的距离
                var distance = xLeft - left;//鼠标点击时到进度条左侧的距离
