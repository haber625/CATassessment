...
function get_false(url, data, success) {
    var xhr = new XMLHttpRequest;
    if (data) {
        url += '?' + data;
    }
...
window.hotList = new Array;
//获取热门歌单
get('http://localhost:3000/playlist/hot', '', function () {
})
//获取热门歌单中的华语歌单的8首歌曲
get_false('http://localhost:3000/playlist/detail?id=5001', '', function (data) {
    for (var i = 0; i < 8; i++) {
...
//热门推荐8个小li
for (var i = 0; i < h_song.length; i++) {
    h_song[i].onclick = function () {
        get('http://localhost:3000/song/url', '' + 'id=' + hotList[i], function (data) {
