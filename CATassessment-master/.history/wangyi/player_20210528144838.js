...
get('http://localhost:3000/playlist/detail?id=19723756', '', function (data) {
    ...
    for (var i = 0; i < 10; i++) {
        ...
        //记录id
        song_rise[i].href = "" + "id=" + data.playlist.tracks[i].id;
        //根据id获取url
        song_rise[i].addEventListener('click', function () {
            get('http://localhost:3000/song/url', '' + song_rise[i].href, function (data) {
...
for (var i = 0; i < 10; i++) {
    song_rise[i].addEventListener('click', function () { console.log(song_rise[i]); })
}
